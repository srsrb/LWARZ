import{WIDTH,players} from '../global/glb_var.js';
...
function scroll1(){
    if(players[0].x +players[0].height >= WIDTH/2 && players[0].x > players[1].x){
        if(players[0].vx > 0.5 ){
            vscroll = Math.ceil(players[0].vx-1)+0.5;
        }
    }
    else if(players[1].x +players[1].height >= WIDTH/2 && players[1].x > players[0].x){
        if(players[1].vx > 0.5 ){
